...
noise = 0.2  # noise level, the noise is sampled from N(0, noise)
truth = []  # truth curve
sample = []  # sample points with noise
delay_step = 4  # delay steps from sample to truth
itr_len = 120  # the steps of each phase

# adding truth curve, this can be flexible if you want to simulate different movement
for i in range(int(1.5 * itr_len), 4 * itr_len):
    x.append(i)
...
plt.plot(x, kf_predict_final, label='prediction')
plt.plot(x, kf_predict_final_a, label='prediction with acceleration')
